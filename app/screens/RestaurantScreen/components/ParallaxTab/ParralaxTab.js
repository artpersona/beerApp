    ...
    tabs.forEach((tab, i) => {
      ...
      if (scrollY > tab.y && scrollY < tab.y + tab.height) {
        ...
        Animated.timing(value, {
          ...
          if (scrollBarScroll)
            setTimeout(
              () => tabScrollRef.current.scrollTo({ x: scrollTo }),
              200
            );
        });
        ...
        >
          <View>
            <View style={[style.row]}>
              {props.tabs &&
                props.tabs.map((food, i) => (
                  <TouchableWithoutFeedback
                    key={i}
                    onPress={(e) => handleScroll(food.title)}
                    onLayout={(e) =>
                      handleTab(food.title, e.nativeEvent.layout)
                    }
                  >
                    <Text style={[style.tab]}>{food.title}</Text>
                  </TouchableWithoutFeedback>
                ))}
            </View>
            ...
            <Animated.View style={[{ translateX: value }]}>
              <Animated.View
                style={[style.indicator, { width: widthValue }]}
              ></Animated.View>
            </Animated.View>
          </View>
        </ScrollView>
        ...
        <View style={{ paddingHorizontal: 15, backgroundColor: "white" }}>
          {props.tabs &&
            props.tabs.map((food) => (
              <View
                onLayout={(e) =>
                  handleTabContent(food.title, e.nativeEvent.layout)
                }
              >
                <Text style={style.sectionTitle}>{food.title}</Text>
                <View style={props.itemsContainerStyle || {}}>
                  {food[props.itemListPropertyName].map((singleFood, index) => {
                    return (
                      props.renderItem &&
                      props.renderItem({ item: singleFood, index })
                    );
                  })}
                </View>
              </View>
